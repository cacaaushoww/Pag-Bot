...
# Origem permitida pro painel — em produção isso deve ser o domínio
# real do seu front-end, NUNCA "*", porque agora as rotas usam cookie
# de sessão (CORS com credenciais + wildcard é proibido pelo browser
# mesmo, mas deixar explícito evita erro de configuração).
...
def api_auth_callback():
    ...
    if not code or not state or not expected_state or state != expected_state:
        return redirect(f"{FRONTEND_ORIGIN}?login=erro&motivo=state_invalido")
    ...
    try:
        ...
        resp = redirect(f"{FRONTEND_ORIGIN}?login=sucesso")
        ...
        return redirect(f"{FRONTEND_ORIGIN}?login=erro&motivo=falha_discord")
