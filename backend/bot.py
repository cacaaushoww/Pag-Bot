...
def api_bot_name():
    ...
    try:
        # Usando loop do bot para rodar a corotina de forma segura
        future = asyncio.run_coroutine_threadsafe(guild.me.edit(nick=new_name), bot.loop)
        future.result(timeout=10) # Espera o resultado
...
def api_config():
    ...
    if not supabase:
        return {"ok": False, "error": "Supabase não configurado no servidor"}, 500
...
if __name__ == "__main__":
    ...
    if DISCORD_BOT_TOKEN:
        ...
        # Mantém o processo vivo para o Flask continuar rodando
        while True:
