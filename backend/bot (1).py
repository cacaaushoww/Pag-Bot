...
def ler_settings():
    """Lê o bloco 'settings' do database.json. Retorna {} em caso de erro."""
    try:
...
async def criar_pix(interaction: discord.Interaction, valor: float, descricao: str, email: str = None):
    ...
    # --- MÉTODO: PIX DIRETO (sem Mercado Pago) ---
    if metodo_ativo == "pix":
        ...
        if qr_bytes:
            import io
            arquivo_qr = discord.File(io.BytesIO(qr_bytes), filename="qrcode_pix.png")
        ...
        print(f"✅ PIX direto criado com sucesso (método: pix)")
        return
...
def api_config():
    """Salva chave PIX, token do Mercado Pago e/ou método de pagamento ativo no database.json."""
    ...
    try:
        ...
        if 'pix_key' in data:
            ...
            print(f"✅ Chave PIX atualizada: {data['pix_key']}")

        if 'mp_access_token' in data:
            db['settings']['mp_access_token'] = data['mp_access_token']
            # Atualiza o token do processor em tempo real, sem reiniciar o bot
            payment_processor.access_token = data['mp_access_token']
            print(f"✅ MP Access Token atualizado.")

        if 'mp_pix_key' in data:
            db['settings']['mp_pix_key'] = data['mp_pix_key']
            print(f"✅ Chave PIX do MP atualizada: {data['mp_pix_key']}")
        ...
        if 'payment_method_active' in data:
            ...
            print(f"✅ Método de pagamento ativo alterado para: {metodo}")
...
def api_payment_method():
    """Retorna qual método de pagamento está ativo no momento ('pix' ou 'mercadopago')."""
    settings = ler_settings()
