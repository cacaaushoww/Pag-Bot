...
function abrirConfigPix() {
    ...
    showModal('Configurar PIX', `
        <div class="form-group">
            ...
            <input type="text" id="pixKeyInput" placeholder="CPF, email, telefone ou chave aleatória" value="${saved.pix_key || ''}">
        </div>
        <p style="font-size:12px;color:var(--text-secondary);margin-bottom:16px;">
            O nome do recebedor e a cidade exibidos no código PIX usam valores padrão (VENDABOT / SAO PAULO) por enquanto.
        </p>
...
async function testarTokenMP() {
    ...
    try {
        ...
        if (res.ok && data.email) {
            ...
            const msg = data.message || 'Token inválido ou sem permissão.';
            resultEl.style.background = 'rgba(255,107,107,0.1)';
            resultEl.style.color = 'var(--danger)';
...
/* ===========================
   MÉTODO DE PAGAMENTO ATIVO (PIX x MERCADO PAGO)
   =========================== */

// Aplica visualmente qual card está marcado como "ativo" (borda verde + status)
...
// Carrega do bot/localStorage qual método está ativo e marca o switch correspondente
async function loadPaymentMethodActive() {
    ...
    try {
        ...
        if (data.ok && data.active) {
            metodoAtivo = data.active;
        }
    } catch (e) {
        // Bot offline: usa o que estiver salvo localmente, senão o padrão
        const saved = JSON.parse(localStorage.getItem('pagbot_settings') || '{}');
        metodoAtivo = saved.payment_method_active || 'mercadopago';
        console.log("[v0] Bot offline, usando método salvo localmente:", metodoAtivo);
    }
...
// Define qual método fica ativo, desativa o outro e salva (local + bot)
async function setPaymentMethodActive(metodo) {
    ...
    try {
        ...
        const data = await res.json();
        if (data.ok) {
            showToast(metodo === 'pix' ? 'PIX ativado! Mercado Pago foi desativado.' : 'Mercado Pago ativado! PIX foi desativado.');
        } else {
            showToast('Salvo localmente, mas o bot recusou: ' + (data.error || 'erro'));
        }
    } catch (e) {
        console.log("[v0] Bot offline, método de pagamento salvo apenas localmente.");
        showToast('Bot offline: método salvo localmente, será sincronizado quando o bot voltar.');
    }
}

// Liga os dois switches em modo "rádio": ativar um desativa o outro
function setupPaymentMethodToggles() {
    ...
    toggles.forEach(toggle => {
        toggle.addEventListener('change', (e) => {
            ...
            if (e.target.checked) {
                // Usuário ativou este método -> ele se torna o único ativo
                setPaymentMethodActive(metodo);
            } else {
                // Usuário tentou desativar o que já está ativo -> o outro assume
                const outroMetodo = metodo === 'pix' ? 'mercadopago' : 'pix';
                setPaymentMethodActive(outroMetodo);
            }
        });
    });
}
